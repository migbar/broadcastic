...
describe Broadcastic::Callbacks do
	...
	context "with a single event" do
		context "without options" do
			...
			it "sets up the creation callbacks when invoked from the class definition" do
			...
			it "sets up the update callbacks when invoked from the class definition" do
			...
			it "sets up the destroy callbacks when invoked from the class definition" do
			...
			it "sets up all callbacks when :changes are invoked from the class definition" do
				Broadcastic::Callbacks.should_receive(:broadcast_creations).with(Product, no_options)
				Broadcastic::Callbacks.should_receive(:broadcast_updates).with(Product, no_options)
				Broadcastic::Callbacks.should_receive(:broadcast_destroys).with(Product, no_options)
		...
		context "with :to option"	do
			...
			it "sets up the callbacks with the correct destination" do
	...
	context "with multiple events" do
		context "without options" do
			...
			it "sets up the callbacks for each event when invoked from the class definition" do
				Broadcastic::Callbacks.should_receive(:broadcast_creations).with(Product, no_options)
				Broadcastic::Callbacks.should_receive(:broadcast_updates).with(Product, no_options)
